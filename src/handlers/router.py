...
async def proactive_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Checks time and task status to propose actions.
    """
    user_id = update.effective_user.id
    user = db.get_user_profile(user_id)
    
    if not user:
        # Not registered? Forward to Onboarding
    ...
    # Simple Time Check (assuming Dubai/UAE time for now, or user server time)
    # Ideally use user.p_time but let's do rough blocks
    ...
    if 6 <= now_hour < 14 and not morning_done:
        suggestion = f"☀️ Your **Morning Check-in** for {len(user.landmarks)} spots is pending."
    elif now_hour >= 16 and not evening_done:
        suggestion = "🌙 Ready to record your **Evening Summary**?"
        if not morning_done:
            suggestion = "🚜 You have **Morning & Evening** tasks pending today."
